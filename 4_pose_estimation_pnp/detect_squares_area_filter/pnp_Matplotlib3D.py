...
"""
================================================================================
3D Pose Visualization + Camera Trajectory Visualization
================================================================================

功能说明:
    - 固定世界坐标系建系：物理坐标轴始终正交，X/Y/Z方向固定。
    - 检测黄色背景上的黑色方块，获取四个角点进行 SolvePnP 姿态估计。
    - 绘制单帧相机坐标轴投影到图像上（X红, Y绿, Z蓝）以及角点重投影点。
    - 输出重投影误差、旋转矩阵正交性和欧拉角。
    - 记录每一帧相机在世界坐标系下的平移向量 `tvec`，生成 **相机移动轨迹图**。
      可用于验证相机运动连续性、位姿稳定性和路径规划分析。

对比优化版：
    - 优化版：使用上一帧姿态初始化 + 平滑处理，适合实时跟踪。
    - 当前版本：坐标系固定，轨迹可量化，便于物理验证和轨迹分析。

角点顺序:
    左上 → 右上 → 右下 → 左下
    对应物理世界坐标 objectPoints:
        [-75,  75, 0] 左上
        [ 75,  75, 0] 右上
        [ 75, -75, 0] 右下
        [-75, -75, 0] 左下
...
输入/输出:
    - 输入视频: stone.mp4
    - 输出视频: ./results/output_result_时间戳.mp4
    - 输出相机移动轨迹图: ./results/camera_trajectory_时间戳.png
    - 可通过 's' 键保存当前帧图像

版本记录:
    1.0 - 固定世界坐标系 + 相机轨迹可视化
    1.1 - 优化版: 上一帧姿态初始化 + 平滑处理，适合实时跟踪
"""
