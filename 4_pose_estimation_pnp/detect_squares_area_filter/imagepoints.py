...
def detect_black_shapes_on_yellow(input_data, prev_squares=None, jump_threshold=10, area_diff_ratio=0.5):
    ...
    input_data: str 或 ndarray
        输入图片路径或 OpenCV 图像数组
    prev_squares: 上一帧检测到的四个方块坐标，用于跳动过滤
    jump_threshold: 跳动阈值（像素），上一帧坐标跳动超过该值则可能被过滤
    area_diff_ratio: 面积差异阈值比例，如果当前方块与上一帧面积差异超过这个比例，则不输出该坐标
    """
    rectangles = []  # 用于存放检测到的黑色方块坐标
    ...
    yellow_roi = img[y:y + h, x:x + w]  # 提取黄色区域

    # 将黄色区域转换为 HSV 空间，准备检测黑色
